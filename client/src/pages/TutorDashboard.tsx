...
export default function TutorDashboard() {
  ...
  const [checkingInstances, setCheckingInstances] = useState<Record<number, boolean>>({});
  ...
  const handleFetchTranscript = async (sessionId: number, joinUrl: string | null, scheduledAt?: number | null) => {
    ...
    // Extract meeting ID from Zoom URL (supports /j/, /s/, /join/, ?confno= formats)
    const meetingId = joinUrl.match(/\/j\/(\d+)/)?.[1]
      ?? joinUrl.match(/\/s\/(\d+)/)?.[1]
      ?? joinUrl.match(/\/join\/(\d+)/)?.[1]
      ?? joinUrl.match(/[?&]confno=(\d+)/)?.[1]
      ?? null;
	      ...
	      <Dialog open={!!fixTranscriptSession && recordingInstances.length > 1} onOpenChange={open => { if (!open) { setFixTranscriptSession(null); setRecordingInstances([]); setFixSelectedUuid(""); } }}>
	        <DialogContent className="max-w-lg">
	          ...
	          <DialogFooter>
	            ...
	            >
	              {fetchTranscriptMutation.isPending ? "Re-fetching…" : "Re-fetch Transcript"}
	            </Button>
	          </DialogFooter>
	        </DialogContent>
	      </Dialog>
