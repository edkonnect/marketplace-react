...
// ── Test Prep sub-groups (India nav flyout: SAT/ACT, IELTS, TOEFL, GRE/GMAT) ─
export const TEST_PREP_GROUPS = ["SAT/ACT", "IELTS", "TOEFL", "GRE/GMAT"] as const;
export type TestPrepGroup = (typeof TEST_PREP_GROUPS)[number];

export function getTestPrepGroup(title: string): TestPrepGroup | null {
...
export function getTestPrepGroupTitles(group: TestPrepGroup): string[] {
  return CATEGORY_COURSE_TITLES["Test Prep"].filter((t) => getTestPrepGroup(t) === group);
}

// Categories highlighted with a star in the India nav (high-demand programs).
