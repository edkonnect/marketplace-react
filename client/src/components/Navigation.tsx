...
const US_MENU_ITEMS = [
  { label: "Elementary (K-5)",    href: "/courses?gradeLevel=Elementary%20(K-5)" },
  { label: "Middle School (6-8)", href: "/courses?gradeLevel=Middle%20School%20(6-8)" },
  { label: "High School (9-12)",  href: "/courses?gradeLevel=High%20School%20(9-12)" },
  { label: "SAT / ACT Prep",      href: "/courses?category=Test%20Prep" },
  { label: "AP Courses",          href: "/courses?category=AP" },
  { label: "Coding",              href: "/courses?category=Computer%20Science" },
];
...
// India mega menu shows every board/program except Test Prep (which gets its
// own dedicated, starred section up top).
...
export default function Navigation() {
  ...
  // its top content cut off (visible bug in the India mega menu).
  React.useEffect(() => {
              ...
              {role === "coordinator" ? (
                ...
                <>
                  ...
                  >
                    ...
                    {coursesMenuOpen && (
                      <div
                        className={`absolute left-1/2 top-full -translate-x-1/2 pt-3 ${isIndian ? "w-[800px]" : "w-72"}`}
                        onMouseEnter={openCoursesMenu}
                        onMouseLeave={scheduleCloseCoursesMenu}
                      >
                        <div className="rounded-xl border border-border bg-white dark:bg-slate-900 shadow-2xl p-6 overflow-hidden">
                          {isIndian ? (
                            /* ── India: nav list (Test Prep → Board) | detail pane | Grade Level ── */
                            <div className="flex gap-6 items-stretch">
                              ...
                              <div className="w-56 shrink-0 max-h-[26rem] overflow-y-auto pr-1">
                                ...
                                <Link
                                  href="/courses?category=Test%20Prep"
                                  onClick={closeMenusAnd()}
                                  className="mt-2 mb-4 inline-block text-xs font-medium text-primary hover:underline"
                                >
                                  View all Test Prep →
                                </Link>
                            ...
                            /* ── US: single column clean layout ── */
                            <div className="w-56">
                              <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-3">
                                Browse by Topic
                              </p>
                              <div className="flex flex-col gap-0.5">
                                {US_MENU_ITEMS.map((item) => (
                                  <Link
                                    key={item.label}
                                    href={item.href}
                                    onClick={closeMenusAnd()}
