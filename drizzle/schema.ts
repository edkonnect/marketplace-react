...
export const tutorProfiles = mysqlTable("tutor_profiles", {
  ...
  zoomJoinUrl: text("zoomJoinUrl"), // URL students/parents use to join
  zoomHostUrl: text("zoomHostUrl"), // URL tutor uses to start meeting
  zoomMeetingPassword: varchar("zoomMeetingPassword", { length: 50 }), // Optional passcode
...
export const sessionRatings = mysqlTable("session_ratings", {
  ...
  parentId: int("parentId").notNull().references(() => users.id, { onDelete: "cascade" }),
  tutorId: int("tutorId").notNull().references(() => users.id, { onDelete: "cascade" }),
  rating: int("rating").notNull(), // 1-4 (Poor, Average, Good, Excellent)
...
/**
 * Zoom meeting recordings table
 * Stores cloud recording metadata and transcripts from Zoom meetings
 */
export const zoomMeetingRecordings = mysqlTable("zoom_meeting_recordings", {
  id: varchar("id", { length: 255 }).primaryKey(), // Zoom recording UUID
  ...
  transcriptText: mediumtext("transcriptText"), // Plain text transcript
  rawTranscript: mediumtext("rawTranscript"), // VTT format transcript
  durationMinutes: int("durationMinutes"),
  recordedAt: timestamp("recordedAt"),
  processedAt: timestamp("processedAt"),
  status: mysqlEnum("status", ["pending", "processing", "completed", "failed"]).default("pending").notNull(),
  errorMessage: text("errorMessage"),
  shareUrl: text("shareUrl"), // Zoom share URL for recordings
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  meetingIdIdx: index("zoom_recordings_meetingId_idx").on(table.meetingId),
  sessionIdIdx: index("zoom_recordings_sessionId_idx").on(table.sessionId),
  statusIdx: index("zoom_recordings_status_idx").on(table.status),
}));

export type ZoomMeetingRecording = typeof zoomMeetingRecordings.$inferSelect;
export type InsertZoomMeetingRecording = typeof zoomMeetingRecordings.$inferInsert;

/**
 * AI-generated insights from session transcripts
 * Stores summaries, homework, and progress reports generated by Gemini AI
 */
export const sessionAIInsights = mysqlTable("session_ai_insights", {
  ...
  studentProgress: mediumtext("studentProgress"), // AI-generated progress assessment
...
export type SessionAIInsight = typeof sessionAIInsights.$inferSelect;
export type InsertSessionAIInsight = typeof sessionAIInsights.$inferInsert;

// Relations for Zoom tables
export const zoomMeetingRecordingsRelations = relations(zoomMeetingRecordings, ({ one, many }) => ({
  session: one(sessions, {
    fields: [zoomMeetingRecordings.sessionId],
    references: [sessions.id],
  }),
  aiInsights: many(sessionAIInsights),
}));

export const sessionAIInsightsRelations = relations(sessionAIInsights, ({ one }) => ({
  recording: one(zoomMeetingRecordings, {
    fields: [sessionAIInsights.recordingId],
    references: [zoomMeetingRecordings.id],
  }),
  session: one(sessions, {
    fields: [sessionAIInsights.sessionId],
    references: [sessions.id],
  }),
}));
